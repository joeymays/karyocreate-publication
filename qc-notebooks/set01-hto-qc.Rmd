---
title: "KaryoCreate scRNAseq Experiment Set01 - HTO Demultiplexing"
author: "Joey Mays"
output:
  html_notebook:
    code_folding: show
...
KaryoCreate scRNA-seq Experiment Set 01.
...
```{r}
here::i_am("set01-hto-qc.Rmd")
exp.folder <- "scrnaseq_set01"
```
...
sample.metadata <- read.csv(here(exp.folder, "set01-scrnaseq-sample-metadata.csv"))
```
...
```{r include=FALSE}
#print the total HTO counts across all barcodes (internal table)
rowSums(sc.hto)
```
...
The `HTO_Classification.global` attribute indicates whether each cell barcode represents a single, doublet, or unlabeled cell.
...
```{r echo=TRUE}
kable(sc@meta.data %>% filter(HTO_classification.global == "Singlet") %>% group_by(HTO_classification) %>% tally(), table.attr = "style = \"color: black;\"", format.args = list(big.mark = ","),
      caption = "Singlets per HTO (CellRanger Filtered, Pre-QC)", align = "ll", col.names = c("HTO", "No. of Cells")) %>%
  kable_styling(bootstrap_options = c("striped", "bordered", full_width = F, position = "left"), )
```
...
singlet.bcs <- WhichCells(sc, expression = HTO_classification.global == "Negative", invert = T)
#subset Seurat object by singlet barcodes and group by HTO classification call
RidgePlot(sc[,singlet.bcs], assay = "HTO", features = rownames(sc[["HTO"]]), ncol = 5, group.by = "hash.ID")
```
...
```{r}
# determine the percentage of each cell's transcriptome that is attributable to mitochondrial counts
DefaultAssay(sc) <- "RNA"
...
VlnPlot(sc, features = c("nCount_RNA"), ncol = 1, group.by = "hash.ID", pt.size = 0.01, log = T) + geom_hline(yintercept = 30000, color = "red") + geom_hline(yintercept = 3000, color = "red", linetype = "dashed")
VlnPlot(sc, features = c("nFeature_RNA"), ncol = 1, group.by = "hash.ID", pt.size = 0.01, log = T) + geom_hline(yintercept = 800, color = "red", linetype = "dashed")
...
FeatureScatter(sc, feature1 = "nCount_RNA", feature2 = "percent.mt") + geom_hline(yintercept = 10, color = "red") + geom_vline(xintercept = 3000, color = "red", linetype = "dashed")
...
```{r}
sc.subset <- subset(sc, subset = nFeature_RNA > 800 & nCount_RNA < 30000 & nCount_RNA > 3000 & percent.mt < 10 & nCount_HTO < 1500)
```
...
FeatureScatter(sc.subset, feature1 = "nCount_RNA", feature2 = "nCount_HTO_log")
FeatureScatter(sc.subset, feature1 = "nCount_RNA", feature2 = "nCount_HTO_log", group.by = "HTO_classification.global")
...
```{r}
#replotting the HTO heatmap after QC
...
```{r fig.height=8, fig.width=14, message=FALSE}
#pull out singlet barcodes
singlet.bcs <- WhichCells(sc.subset, expression = HTO_classification.global == "Negative", invert = T)
#subset Seurat object by singlet barcodes and group by HTO classification call
RidgePlot(sc.subset[,singlet.bcs], assay = "HTO", features = rownames(sc[["HTO"]]), ncol = 5, group.by = "hash.ID")
```
...
saveRDS(sc.subset, file = here(exp.folder, "sc.Seurat.HTOdeconvolved.filtered.RDS"))
```
